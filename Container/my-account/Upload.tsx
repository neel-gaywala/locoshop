...
export function Upload(ownerUid: string) {
  ...
  const getUploadsHistory = useCallback(async () => {
    if (store) {
      const req = await axios(
        `${process.env.NEXT_PUBLIC_API_ENDPOINT}/upload/history/${store.place_id}`
      );
      return req.data;
    }
    return null;
  ...
  useEffect(() => {
    (async () => {
      const store: StoreType = (
        await axios(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/store/user/${ownerUid}`)
      ).data;
      setStore(store);
    })();
  }, [ownerUid]);
  ...
  async function sendUpload() {
    ...
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/upload`, data, {
        headers: {
